...
def create_datatype(df, train_value, test_value, datatype_var, 
    ...
    # Performs stratified random sample of subset dataframe 
    # to create new df with subset values 
    df_sampled = df_subset.groupby(['Task', compare_dfcolumn],
    ...
    for index in df_subset.index:
        # Labels all datatype_var columns in df 
        # with train_value/test_value based upon 
...
def create_text_column(df, file_directory='data/'):
    '''Reads in the files, listed in a df and returns that df with an additional 
       column, `Text`. 
